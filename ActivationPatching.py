...
from evaluation import logits_to_logit_diff
from IOI_Dataset import IOIExample
from functools import partial
from jaxtyping import Float
from visualization import plot_circuit_graph, plot_activation_importance
...
class ActivationPatching:
    ...
    def __init__(self, model: HookedTransformer, dataset: List[IOIExample]):
        ...
        self.activation_types = ['resid_pre'] #, 'resid_post', 'hook_z', 'mlp_out']
        ...
        while iteration < max_iter:
            ...
            for layer in range(self.model.cfg.n_layers):
                for position in range(num_tokens):
                    if (layer, position) in self.dropped_activations:
                        continue
                    node = CircuitNode(layer, position, self.activation_types[0])
                    ...
                    if avg_score > self.threshold:
                        ...
                        self.dropped_activations.add((layer, position))
